...
const UserPayments = () => {
  ...
  const userAppointmentIDs =
    userAppointmentsData?.appointments?.map((appt: TAppointment) => appt.appointmentID) || [];
  ...
  // ✅ Add this to fix missing prop error
  const handlePaymentCreated = (
    paymentID: number,
    amount: number,
    phoneNumber: string
  ) => {
    toast.success(`Payment #${paymentID} created successfully.`);
    console.log("User payment created:", { paymentID, amount, phoneNumber });
  };
  ...
  const filteredPayments =
    paymentsData?.payments?.filter((p) => userAppointmentIDs.includes(p.appointmentID)) || [];
